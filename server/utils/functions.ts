...
export function getPluginService<T extends NavigationServiceName>(
  name: T
): T extends infer R extends NavigationServiceName ? TypeMap[R] : never {
  return strapi.plugin("navigation").service(name);
}

export const errorHandler =
  (ctx: ToBeFixed) => (error: NavigationError | string) => {
...
export const templateNameFactory = async (
  ...
  const relatedMap = flatRelated.reduce(
    (acc: { [key: string]: Id[] }, curr) => {
  ...
  const responses = await Promise.all(
    Object.entries(relatedMap).map(([contentType, ids]) => {
      ...
      return strapi
        .query<ContentTypeEntity>(contentType)
        .findMany({
          where: { id: { $in: ids } },
  ...
  return (contentType: ToBeFixed, id: Id) => {
    ...
    if (template && template instanceof Array) {
      const templateComponent = getTemplateComponentFromTemplate(
        strapi,
        template
      );
...
export const prepareAuditLog = (actions: NavigationActions[]): string => {
  return [
    ...new Set(
      actions
...
export const composeItemTitle = (
  ...
  if (title) {
    ...
    const relationTitle = extractItemRelationTitle(
      lastRelated,
      fields,
      contentTypes
    );
    return isString(relationTitle) && !isEmpty(relationTitle)
      ? relationTitle
      : undefined;
  }
  return undefined;
...
export const extractItemRelationTitle = (
  ...
  const contentType = find(
    contentTypes,
    (_) => _.contentTypeName === __contentType
  );
...
export const filterOutUnpublished = (
  ...
  const relatedItem =
    item.related && (isArray(item.related) ? last(item.related) : item.related);
      ...
      for (let item of checkData) {
        for (let _ of parentItem.items) {
          if (
            ...
            return reject(
              new NavigationError(
                `Duplicate path:${item.path} in parent: ${
                  parentItem.title || "root"
                } for ${item.title} and ${_.title} items`,
                {
...
export const buildNestedPaths = <
  T extends Pick<NavigationItemEntity, "parent" | "id" | "path">
>(
  items: T[],
  id: Id | null = null,
  ...
  return items
    .filter((entity) => {
      ...
      return (
        (data && data === id) ||
        (isObject(entity.parent) && entity.parent.id === id)
      );
    })
...
>(
  ...
  const itemsWithPaths = path
    ? buildNestedPaths(items).filter(({ path: itemPath }) =>
...
export const isContentTypeEligible = (uid = "") => {
  const isOneOfAllowedType =
    ALLOWED_CONTENT_TYPES.filter((_) => uid.includes(_)).length > 0;
  const isNoneOfRestricted =
    RESTRICTED_CONTENT_TYPES.filter((_) => uid.includes(_) || uid === _)
      .length === 0;
...
export const intercalate = <T, U extends T>(glue: T, arr: U[]) =>
  arr
    .slice(1)
    .reduce<Array<T | U>>(
      (acc, element) => acc.concat([glue, element]),
      arr.slice(0, 1)
    );
...
export const getPluginModels = (): Record<
...
export const validateAdditionalFields = (
  ...
  if (
    !isNil(find(customFields, (item) => includes(forbiddenNames, item.name)))
  ) {
    throw new Error(
      `Name of custom field cannot be one of: ${forbiddenNames.join(", ")}`
    );
  }
};

export const parsePopulateQuery = (
...
export const sanitizePopulateField = (populate: Populate): Populate => {
  ...
  if (Array.isArray(populate)) {
    return populate.map((item): Populate => sanitizePopulateField(item));
  }
