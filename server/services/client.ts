...
const clientService: (context: StrapiContext) => IClientService = ({
  strapi,
}) => ({
  async readAll({ locale, orderBy = "createdAt", orderDirection = "DESC" }) {
    ...
    let navigations = await strapi.query<Navigation>(masterModel.uid).findMany({
      where: locale
        ? {
            localeCode: locale,
          }
    ...
    if (i18nEnabled) {
      navigations = navigations.reduce((acc, navigation) => {
        if (navigation.localeCode && locales?.includes(navigation.localeCode)) {
          acc.push({
            ...navigation,
            localizations: navigation.localizations?.filter(
              ({ localeCode }) => localeCode && locales?.includes(localeCode)
            ),
          });
        }
  ...
  renderRFRNav(item): RFRNavItem {
    ...
    if (type === "EXTERNAL") {
      assertNotEmpty(
        path,
        new NavigationError(
          "External navigation item's path is undefined",
          item
        )
      );
      return {
  ...
  async renderType({
    ...
    const entity = await i18nAwareEntityReadHandler({
      entity: await strapi.query<Navigation>(masterModel.uid).findOne({
    ...
    if (entity && entity.id) {
      ...
      const { contentTypes, contentTypesNameFields, additionalFields } =
        await adminService.config(false);
      const enabledCustomFieldsNames = getCustomFields(additionalFields).reduce<
        string[]
      >((acc, curr) => (curr.enabled ? [...acc, curr.name] : acc), []);

      const wrapContentType = (itemContentType: ToBeFixed) =>
      ...
      switch (type) {
        ...
        case RENDER_TYPES.RFR:
          const getTemplateName = await templateNameFactory(
            items,
            strapi,
            contentTypes
          );
          const itemParser = async (
            item: NavigationItemEntity<ContentTypeEntity[]>,
            path = "",
            field: keyof NavigationItemEntity
          ) => {
            ...
            const slug = isString(parentPath)
              ...
              : undefined;
            const lastRelated = isArray(item.related)
              ? last(item.related)
              : item.related;
            ...
            const customFields = enabledCustomFieldsNames.reduce(
              (acc, field) => {
                const mapper =
                  customFieldsDefinitions.find(({ name }) => name === field)
                    ?.type === "media"
                ...
                return {
                  ...acc,
                  [field]: content ? mapper(content as string) : content,
                };
              },
              {}
            );
            ...
            return {
                ...
                isExternal || !lastRelated
                  ...
                  : {
                      ...
                      __templateName: getTemplateName(
                        lastRelated.relatedType || lastRelated.__contentType,
                        lastRelated.id
                      ),
                    },
              ...
              items: isExternal
                ? undefined
                : await clientService.renderTree(
                    items,
                    item.id,
                    field,
                    parentPath,
          ...
          const { items: itemsFilteredByPath, root: rootElement } =
          ...
          const filteredStructure = filter
            ? treeStructure.filter(
                (item: NavigationItem) => item.uiRouterKey === filter
              )
            : treeStructure;
        ...
        default:
          ...
          const getNestedOrders = (
            id: Id,
            cache: Map<Id, Array<number>> = defaultCache
          ): Array<number> => {
          ...
          return result
            .map(
              ({
                additionalFields,
                ...item
              }: NavigationItemEntity<ContentTypeEntity>) => {
                const customFields = enabledCustomFieldsNames.reduce(
                  (acc, field) => {
                    const mapper =
                      customFieldsDefinitions.find(({ name }) => name === field)
                        ?.type === "media"
                        ? (_: string | boolean) =>
                            pickMediaFields(JSON.parse(_.toString()))
                        : identity;
            ...
            .sort((a, b) =>
              compareArraysOfNumbers(
                getNestedOrders(a.id),
                getNestedOrders(b.id)
              )
            );
      }
    }
