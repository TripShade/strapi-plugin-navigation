...
export type NavigationServiceName =
  | "common"
  | "admin"
  | "client"
  | "navigation-item";
...
export interface IAdminService {
  ...
  readNavigationItemFromLocale(payload: {
    ...
    Partial<
      Pick<
        NotVoid<Navigation["items"]>[number],
        "path" | "related" | "type" | "uiRouterKey" | "title" | "externalPath"
      >
    >
  >;
}
...
export interface ICommonService {
  analyzeBranch: (
    ...
    parentItem?: ToBeFixed,
    prevOperations?: NavigationActions
  ) => Promise<NavigationActionsPerItem[]>;
  configContentTypes: (
    viaSettingsPage?: boolean
  ) => Promise<StrapiContentType<ToBeFixed>[]>;
  createBranch: (
    items: NestedStructure<NavigationItemInput | NavigationItem>[],
    masterEntity: Navigation | null,
  ...
  getIdsRelated: (
    relatedItems: RelatedRefBase[] | RelatedRef[] | null,
    master: number
  ) => Promise<Id[] | void>;
  getPluginStore: () => Promise<StrapiStore>;
  getRelatedItems: (
    entityItems: NavigationItemEntity[],
    populate?: PopulateQueryParam
  ) => Promise<NavigationItemEntity<ContentTypeEntity>[]>;
  pruneCustomFields: (
...
export interface IClientService {
  readAll: (
    input: { orderBy?: string; orderDirection?: string } & I18nQueryParams
  ) => Promise<ToBeFixed>;
  render: (
    input: {
      idOrSlug: Id;
